...
export const load: PageServerLoad = async ({ params }) => {
	...
	// Get all themes for this year that are lectures
	...
	>`
		...
		WHERE YEAR(e_themes.date_new) = ${year} AND lecwp = 'lectures'
		GROUP BY e_themes.id
		ORDER BY e_themes.date_new ASC, theme ASC
	`;

	// Parse themes into grouped lectures (matching original presentation: grouped by theme with date)
	const themeGroups = themes.map((t) => {
		...
		const lectures = titres.map((titre, i) => ({
			id: parseInt(ids[i]) || 0,
			title: titre || null,
			abstract: null,
			link: null,
