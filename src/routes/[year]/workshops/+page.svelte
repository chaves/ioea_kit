										...
										<a href="/{data.year}/presentation/{workshop.id}" class="no-underline text-inherit block group">
											<div class="bg-white rounded-xl border border-border overflow-hidden flex flex-col h-full shadow-[0_2px_8px_rgba(0,0,0,0.06)] transition-all duration-300 cursor-pointer group-hover:-translate-y-1 group-hover:shadow-[0_8px_24px_rgba(0,0,0,0.12)] group-hover:border-primary">
												...
												<div class="p-5 flex flex-col gap-3 flex-1">
													{#if workshop.title}
														<h4 class="text-primary m-0 text-[1.05rem] font-semibold leading-snug">{workshop.title}</h4>
													{/if}
													{#if workshop.abstract}
														<p class="text-text leading-relaxed m-0 text-[0.95rem]">{@html workshop.abstract}</p>
													{/if}
													{#if workshop.link}
														<button
															type="button"
